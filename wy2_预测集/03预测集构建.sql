DROP TABLE IF EXISTS wy2_train_user_item;
CREATE TABLE wy2_train_user_item AS
SELECT p.* FROM
(SELECT t.user_id,t.item_id FROM (SELECT user_id,item_id,SUM(dobuy) AS buycnt FROM user_item_detail WHERE dodate='2014-12-18 00:00:00' GROUP BY user_id,item_id )t WHERE buycnt=0)p
JOIN item q
ON p.item_id=q.item_id

DROP TABLE IF EXISTS wy2_train_data;
CREATE TABLE wy2_train_data AS 
SELECT a.*,
...
FROM wy2_train_user_item a 
LEFT OUTER JOIN wy2_f1 b ON a.user_id=b.user_id
LEFT OUTER JOIN wy2_f2 c ON a.item_id=c.item_id
LEFT OUTER JOIN wy2_f3 d ON a.user_id=d.user_id AND a.item_id=d.item_id

DROP TABLE IF EXISTS wy2_train_data_filter;
CREATE TABLE wy2_train_data_filter AS 
SELECT * FROM wy2_train_data WHERE user_totaldays_view_total<25000;