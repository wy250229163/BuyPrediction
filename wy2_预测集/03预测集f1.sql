DROP TABLE IF EXISTS wy2_f1_t1;
CREATE TABLE wy2_f1_t1 AS 
SELECT user_id , 
...
DROP TABLE IF EXISTS wy2_f1_t2;
CREATE TABLE wy2_f1_t2 AS 
SELECT user_id , 
...
DROP TABLE IF EXISTS wy2_f1_t3;
CREATE TABLE wy2_f1_t3 AS 
SELECT user_id , 
...
DROP TABLE IF EXISTS wy2_f1_t4_1;
CREATE TABLE wy2_f1_t4_1 AS 
SELECT user_id , 
...
DROP TABLE IF EXISTS wy2_f1_t4_2;
CREATE TABLE wy2_f1_t4_2 AS 
SELECT user_id , 
...
FROM user_item_detail 
WHERE datediff(to_date("2014-12-19","yyyy-mm-dd"),dodate,"dd")=2
GROUP BY user_id;

DROP TABLE IF EXISTS wy2_f1_t4_3;
CREATE TABLE wy2_f1_t4_3 AS 
SELECT user_id , 
...
FROM user_item_detail 
WHERE datediff(to_date("2014-12-19","yyyy-mm-dd"),dodate,"dd")=3
GROUP BY user_id;

DROP TABLE IF EXISTS wy2_f1_t5;
CREATE TABLE wy2_f1_t5 AS 
...
DROP TABLE IF EXISTS wy2_f1_t6;
CREATE TABLE wy2_f1_t6 AS 
...
(SELECT user_id ,CASE WHEN dohour_type=1 THEN 1 ELSE 0 END AS dawnact,CASE WHEN dohour_type=2 THEN 1 ELSE 0 END AS amact,CASE WHEN dohour_type=3 THEN 1 ELSE 0 END AS pmact
FROM user_item_detail WHERE datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=7 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t7;
CREATE TABLE wy2_f1_t7 AS 
...
(SELECT user_id ,CASE WHEN dohour_type=1 THEN 1 ELSE 0 END AS dawnact,CASE WHEN dohour_type=2 THEN 1 ELSE 0 END AS amact,CASE WHEN dohour_type=3 THEN 1 ELSE 0 END AS pmact
FROM user_item_detail WHERE datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=3 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t8_1;
CREATE TABLE wy2_f1_t8_1 AS
SELECT t.user_id , SUM(t.cnt) AS user_last_1day_totalhour FROM 
(SELECT DISTINCT user_id,dohour,1 AS cnt FROM user_item_detail WHERE dodate=to_date("2014-12-18","yyyy-mm-dd") )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t8_2;
CREATE TABLE wy2_f1_t8_2 AS
SELECT t.user_id , SUM(t.cnt) AS user_last_2day_totalhour FROM 
(SELECT DISTINCT user_id,dohour,1 AS cnt FROM user_item_detail WHERE datediff(to_date("2014-12-19","yyyy-mm-dd"),dodate,"dd")=2 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t8_3;
CREATE TABLE wy2_f1_t8_3 AS
SELECT t.user_id , SUM(t.cnt) AS user_last_3day_totalhour FROM 
(SELECT DISTINCT user_id,dohour,1 AS cnt FROM user_item_detail WHERE datediff(to_date("2014-12-19","yyyy-mm-dd"),dodate,"dd")=3 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t9_1;
CREATE TABLE wy2_f1_t9_1 AS 
...
FROM user_item_detail WHERE dobuy=1 AND datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=7 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t9;
CREATE TABLE wy2_f1_t9 AS
SELECT a.*,
...
FROM wy2_f1_t9_1 a 
LEFT OUTER JOIN 
(SELECT user_id,SUM(dobuy) AS user_last1week_buy_total FROM user_item_detail WHERE dobuy=1 AND datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=7 GROUP BY user_id)b
ON a.user_id=b.user_id;

DROP TABLE IF EXISTS wy2_f1_t9_1_E1;
CREATE TABLE wy2_f1_t9_1_E1 AS 
...
(SELECT user_id,item_id,CASE WHEN dohour_type=1 THEN 1 ELSE 0 END AS dawnbuy,CASE WHEN dohour_type=2 THEN 1 ELSE 0 END AS ambuy,CASE WHEN dohour_type=3 THEN 1 ELSE 0 END AS pmbuy 
FROM user_item_detail WHERE dobuy=1 AND datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=3 )t
GROUP BY t.user_id;

DROP TABLE IF EXISTS wy2_f1_t9_E1;
CREATE TABLE wy2_f1_t9_E1 AS
SELECT a.*,
...
FROM wy2_f1_t9_1_E1 a 
LEFT OUTER JOIN 
(SELECT user_id,SUM(dobuy) AS user_last3days_buy_total FROM user_item_detail WHERE dobuy=1 AND datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=3 GROUP BY user_id)b
ON a.user_id=b.user_id;

DROP TABLE IF EXISTS wy2_f1_t10;
CREATE TABLE wy2_f1_t10 AS 
SELECT t.user_id,datediff(to_date("2014-12-19","yyyy-mm-dd"),t.dodate,"dd") AS user_lastbuy_diffdays FROM 
(SELECT user_id,dodate,row_number() over (partition by user_id order by dodate DESC) AS order_date 
FROM user_item_detail WHERE dobuy=1 AND dodate>=to_date("2014-11-19","yyyy-mm-dd") AND dodate<=to_date("2014-12-18","yyyy-mm-dd")  )t
WHERE t.order_date=1;

DROP TABLE IF EXISTS wy2_f1_t11;
CREATE TABLE wy2_f1_t11 AS 
SELECT t.user_id,datediff(to_date("2014-12-19","yyyy-mm-dd"),t.dodate,"dd") AS user_lastact_diffdays FROM 
(SELECT user_id,dodate,row_number() over (partition by user_id order by dodate DESC) AS order_date 
FROM user_item_detail WHERE dodate>=to_date("2014-11-19","yyyy-mm-dd") AND dodate<=to_date("2014-12-18","yyyy-mm-dd") )t
WHERE t.order_date=1;

DROP TABLE IF EXISTS wy2_f1_t12_1;
CREATE TABLE wy2_f1_t12_1 AS
SELECT user_id,item_id,SUM(doview) AS doview,SUM(dofavor) AS dofavor,SUM(doaddcar) AS doaddcar,SUM(dobuy) AS dobuy 
FROM user_item_detail 
WHERE datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=7
GROUP BY user_id,item_id;

DROP TABLE IF EXISTS wy2_f1_t12;
CREATE TABLE wy2_f1_t12 AS 
SELECT g.*,
...
(
  ...
  (
	...
	FROM wy2_f1_t12_1 a 
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last1week_favor_count FROM wy2_f1_t12_1 WHERE dofavor>0 GROUP BY user_id ) b
	ON a.user_id=b.user_id
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last1week_addcar_count FROM wy2_f1_t12_1 WHERE doaddcar>0 GROUP BY user_id ) c
	ON a.user_id=c.user_id
  ) d
  LEFT OUTER JOIN
	  (SELECT user_id,COUNT(item_id) AS user_last1week_favorandbuy_count FROM wy2_f1_t12_1 WHERE dofavor>0 AND dobuy>0 GROUP BY user_id) e
  ON d.user_id=e.user_id
  LEFT OUTER JOIN
	  (SELECT user_id,COUNT(item_id) AS user_last1week_addcarandbuy_count FROM wy2_f1_t12_1 WHERE doaddcar>0 AND dobuy>0 GROUP BY user_id) f
  ON d.user_id=f.user_id
)g;

DROP TABLE IF EXISTS wy2_f1_t13_1;
CREATE TABLE wy2_f1_t13_1 AS
SELECT user_id,item_id,SUM(doview) AS doview,SUM(dofavor) AS dofavor,SUM(doaddcar) AS doaddcar,SUM(dobuy) AS dobuy 
FROM user_item_detail 
WHERE datediff(to_date("2014-12-18","yyyy-mm-dd"),dodate,"dd")<=3
GROUP BY user_id,item_id;

DROP TABLE IF EXISTS wy2_f1_t13;
CREATE TABLE wy2_f1_t13 AS 
SELECT g.*,
...
(
  ...
  (
	...
	FROM wy2_f1_t12_1 a 
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last3days_favor_count FROM wy2_f1_t12_1 WHERE dofavor>0 GROUP BY user_id ) b
	ON a.user_id=b.user_id
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last3days_addcar_count FROM wy2_f1_t12_1 WHERE doaddcar>0 GROUP BY user_id ) c
	ON a.user_id=c.user_id
	)d
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last3days_favorandbuy_count FROM wy2_f1_t12_1 WHERE dofavor>0 AND dobuy>0 GROUP BY user_id) e
	ON d.user_id=e.user_id
	LEFT OUTER JOIN
	(SELECT user_id,COUNT(item_id) AS user_last3days_addcarandbuy_count FROM wy2_f1_t12_1 WHERE doaddcar>0 AND dobuy>0 GROUP BY user_id) f
	ON d.user_id=f.user_id
)g;

DROP TABLE IF EXISTS wy2_f1_t14;
CREATE TABLE wy2_f1_t14 AS 
SELECT p.user_id,
...
CASE WHEN q.user_totaldays_rebuy_itemcount IS NULL THEN 0
  ...
  (SELECT a.user_id,COUNT(a.item_id)AS user_totaldays_buy_itemcount FROM
	(SELECT DISTINCT user_id,item_id FROM user_item_detail WHERE dobuy>0 AND dodate>=to_date("2014-11-19","yyyy-mm-dd") AND dodate<=to_date("2014-12-18","yyyy-mm-dd") ) a
  ...
  (SELECT c.user_id,COUNT(c.item_id) AS user_totaldays_rebuy_itemcount FROM
	(SELECT b.user_id,b.item_id,SUM(cnt) AS cnt FROM
	  (SELECT user_id,item_id,dodate,1 AS cnt FROM user_item_detail WHERE dobuy>0 AND dodate>=to_date("2014-11-19","yyyy-mm-dd") AND dodate<=to_date("2014-12-18","yyyy-mm-dd")  
...
DROP TABLE IF EXISTS wy2_f1;
CREATE TABLE wy2_f1 AS 
							...
							(
							  ...
							  (
								...
								(
								  ...
								  wy2_f1_t1 a LEFT OUTER JOIN wy2_f1_t2 b
								  ON a.user_id=b.user_id
								) c
								LEFT OUTER JOIN wy2_f1_t3 d
								ON c.user_id=d.user_id
							  ) e
							  LEFT OUTER JOIN wy2_f1_t4_1 f
							  ON e.user_id=f.user_id
							) g
							LEFT OUTER JOIN wy2_f1_t4_2 h
							ON g.user_id=h.user_id
						  ) i
						  LEFT OUTER JOIN wy2_f1_t4_3 j
						  ON i.user_id=j.user_id
						) k
						LEFT OUTER JOIN wy2_f1_t5 l
						ON k.user_id=l.user_id 
					  ) m
					  LEFT OUTER JOIN wy2_f1_t6 n
					  ON m.user_id=n.user_id
					) o
					LEFT OUTER JOIN wy2_f1_t7 p
					ON o.user_id=p.user_id 
				  ) q
				  LEFT OUTER JOIN wy2_f1_t8_1 r
				  ON q.user_id=r.user_id
				) s
				LEFT OUTER JOIN wy2_f1_t8_2 t
				ON s.user_id=t.user_id
			  ) u
			  LEFT OUTER JOIN wy2_f1_t8_3 v
			  ON u.user_id=v.user_id
			) w
			LEFT OUTER JOIN wy2_f1_t9 x
			ON w.user_id=x.user_id 
		  ) y
		  LEFT OUTER JOIN wy2_f1_t9_e1 z
		  ON y.user_id=z.user_id 
		) aa
		LEFT OUTER JOIN wy2_f1_t10 bb
		ON aa.user_id=bb.user_id 
	  ) cc
	  LEFT OUTER JOIN wy2_f1_t11 dd
	  ON cc.user_id=dd.user_id
	) ee
	LEFT OUTER JOIN wy2_f1_t12 ff
	ON ee.user_id=ff.user_id
  ) gg
  LEFT OUTER JOIN wy2_f1_t13 hh
  ON gg.user_id=hh.user_id
) ii
LEFT OUTER JOIN wy2_f1_t14 jj
