DROP TABLE IF EXISTS wy1_f2_t1;
CREATE TABLE wy1_f2_t1 AS
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t2;
CREATE TABLE wy1_f2_t2 AS 
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t3;
CREATE TABLE wy1_f2_t3 AS 
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t4_1;
CREATE TABLE wy1_f2_t4_1 AS 
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t4_2;
CREATE TABLE wy1_f2_t4_2 AS 
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t4_3;
CREATE TABLE wy1_f2_t4_3 AS 
SELECT item_id , 
...
DROP TABLE IF EXISTS wy1_f2_t5;
CREATE TABLE wy1_f2_t5 AS 
...
DROP TABLE IF EXISTS wy1_f2_t6;
CREATE TABLE wy1_f2_t6 AS 
SELECT g.*,
...
(
  ...
  (
	...
	FROM wy1_f1_t12_1 a 
	LEFT OUTER JOIN
	(SELECT item_id,COUNT(user_id) AS item_last1week_favor_count FROM wy1_f1_t12_1 WHERE dofavor>0 GROUP BY item_id ) b
	ON a.item_id=b.item_id
	LEFT OUTER JOIN
	(SELECT item_id,COUNT(user_id) AS item_last1week_addcar_count FROM wy1_f1_t12_1 WHERE doaddcar>0 GROUP BY item_id ) c
	ON a.item_id=c.item_id
  ) d
  LEFT OUTER JOIN
	  (SELECT item_id,COUNT(user_id) AS item_last1week_favorandbuy_count FROM wy1_f1_t12_1 WHERE dofavor>0 AND dobuy>0 GROUP BY item_id) e
  ON d.item_id=e.item_id
  LEFT OUTER JOIN
	  (SELECT item_id,COUNT(user_id) AS item_last1week_addcarandbuy_count FROM wy1_f1_t12_1 WHERE doaddcar>0 AND dobuy>0 GROUP BY item_id) f
  ON d.item_id=f.item_id
)g;

DROP TABLE IF EXISTS wy1_f2_t7;
CREATE TABLE wy1_f2_t7 AS 
SELECT p.item_id,
...
CASE WHEN q.item_totaldays_rebuy_usercount IS NULL THEN 0
  ...
  (SELECT a.item_id,COUNT(user_id) AS item_totaldays_buy_usercount FROM
	  (SELECT DISTINCT item_id,user_id FROM user_item_detail WHERE dobuy>0 AND dodate>=to_date("2014-11-18","yyyy-mm-dd") AND dodate<=to_date("2014-12-17","yyyy-mm-dd")) a
  ...
  (SELECT c.item_id,COUNT(c.user_id) AS item_totaldays_rebuy_usercount FROM
	(SELECT b.item_id,b.user_id,SUM(b.cnt) AS cnt FROM
	  (SELECT item_id,user_id,dodate,1 AS cnt FROM user_item_detail WHERE dobuy>0 AND dodate>=to_date("2014-11-18","yyyy-mm-dd") AND dodate<=to_date("2014-12-17","yyyy-mm-dd") 
...
DROP TABLE IF EXISTS wy1_f2;
CREATE TABLE wy1_f2 AS 
		...
		(
		  ...
		  (
			...
			(
			  ...
			  wy1_f2_t1 a LEFT OUTER JOIN wy1_f2_t2 b
			  ON a.item_id=b.item_id
			) c
			LEFT OUTER JOIN wy1_f2_t3 d
			ON c.item_id=d.item_id
		  ) e
		  LEFT OUTER JOIN wy1_f2_t4_1 f
		  ON e.item_id=f.item_id
		) g
		LEFT OUTER JOIN wy1_f2_t4_2 h
		ON g.item_id=h.item_id
	  ) i
	  LEFT OUTER JOIN wy1_f2_t4_3 j
	  ON i.item_id=j.item_id
	) k
	LEFT OUTER JOIN wy1_f2_t5 l
	ON k.item_id=l.item_id
  ) m
  LEFT OUTER JOIN wy1_f2_t6 n
  ON m.item_id=n.item_id
) o
LEFT OUTER JOIN wy1_f2_t7 p
